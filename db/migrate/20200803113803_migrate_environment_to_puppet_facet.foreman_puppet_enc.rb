class MigrateEnvironmentToPuppetFacet < ActiveRecord::Migration[6.0]
  def up
    ...
    ForemanPuppetEnc::HostgroupPuppetFacet.insert_all(puppet_hostgroups) if puppet_hostgroups.any?
    ...
    ForemanPuppetEnc::HostPuppetFacet.insert_all(puppet_hosts) if puppet_hosts.any?
  end

  def down
    hostgroup_facet_ids = ForemanPuppetEnc::HostgroupPuppetFacet.all.pluck(:hostgroup_id, :environment_id)
    hostgroup_facet_ids.each do |hostgroup_id, env_id|
      Hostgroup.where(id: hostgroup_id).update_all(environment_id: env_id)
    end
    host_facet_ids = ForemanPuppetEnc::HostPuppetFacet.all.pluck(:host_id, :environment_id)

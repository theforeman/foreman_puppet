    ...
    module Host
      ...
      class_methods do
        ...
        def search_by_puppetclass(_key, operator, value)
          ...
          config_group_ids = ForemanPuppetEnc::ConfigGroup.joins(:puppetclasses).where(conditions).pluck(:id)
          host_ids         = ::Host.authorized(:view_hosts, Host).joins(puppet: :puppetclasses).where(conditions).distinct.pluck(:id)
          host_ids        += ForemanPuppetEnc::HostConfigGroup
                             .where(host_type: 'ForemanPuppetEnc::HostPuppetFacet')
